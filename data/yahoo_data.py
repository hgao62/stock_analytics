...
from sqlitedb.models import SP500Holdings,SP500StocksPrice,IndexPrice,WatchListTickers,NASDAQHoldings,StocksPrice
...
def generate_watch_list_report(df: pd.DataFrame,report_date_str:str,report_name:str) -> None:
...
def get_top_gainers(tickers: list, lookback_periods: list, mode: str, all_tickers:List[str], start_date: Optional[pd.Timestamp] = None, end_date: Optional[pd.Timestamp] = None) -> pd.DataFrame:
    ...
    for ticker in tickers:
        ...
        try:
            if ticker not in all_tickers:
                ticker_df = ticker_data_processing('initial', ticker, StocksPrice, start_date, end_date,longest_period)
            else:
                ticker_df = ticker_data_processing(mode, ticker, StocksPrice, start_date, end_date,longest_period)
...
def generate_market_scanner_report(sp500_tickers, lookback_periods: list, increase_thresholds: list, decrease_thresholds: list, args,current_date, report_name:str, all_tickers: List[str]) -> None:
    top_gainers = get_top_gainers(sp500_tickers, lookback_periods, mode=args.mode, all_tickers=all_tickers, start_date=current_date, end_date=current_date+timedelta(days=1))
...
def generate_watchlist_report(tickers, lookback_periods: list, args,current_date, all_tickers:List[str]=None) -> None:
    
    
    top_gainers = get_top_gainers(tickers, lookback_periods, mode=args.mode, all_tickers=all_tickers,start_date=current_date, end_date=current_date+timedelta(days=1))
    ...
    logging.info(f"Watchlist Report for {current_date_str} generated and emailed successfully.")
...
def main():
    ...
    only_nasdaq_tickers = [ticker for ticker in nasdaq_tickers if ticker not in sp500_tickers]
    ...
    for current_date in date_range:
        generate_market_scanner_report(sp500_tickers, lookback_periods, increase_thresholds, decrease_thresholds, args,current_date,report_name="SP500 Market Scanner",all_tickers=all_tickers)
        generate_market_scanner_report(only_nasdaq_tickers, lookback_periods, increase_thresholds, decrease_thresholds, args,current_date, report_name="NASDAQ Market Scanner",all_tickers=all_tickers)
        generate_watchlist_report(watchlist_tickers, lookback_periods, args,current_date, all_tickers)
...
def load_nasdaq_data(sp500_tickers):
    nasdaq_tickers = read_tickers(NASDAQHoldings)
    nasdaq_tickers = [ticker for ticker in nasdaq_tickers if ticker not in sp500_tickers]
